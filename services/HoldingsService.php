...
class HoldingsService
{
    ...
    public function get_holdings(int $user_id): array
    {
        ...
        foreach ($lots_by_instrument as $instrument_id => $lots) {
            ...
            // Fetch buy trades for all lots to get price_per_unit and trade_currency
            $buy_trades = [];
            foreach ($lots as $lot) {
            ...
            // Calculate total shares, cost basis, and weighted average price_per_unit
            $total_price_weighted = '0';
            $total_cost_basis_original = '0.00';
            $cost_currency = null;
            
            // Determine currency from first lot (assume all lots for same instrument use same currency)
            foreach ($lots as $lot) {
                if (isset($buy_trades[$lot->buy_trade_id])) {
                    $cost_currency = $buy_trades[$lot->buy_trade_id]->trade_currency;
                    break;
                }
            }
            ...
            foreach ($lots as $lot) {
                ...
                $lot_remaining_cost = $this->calculate_remaining_cost_basis($lot);
                $total_cost_basis_eur = $this->add_decimals($total_cost_basis_eur, $lot_remaining_cost);
                
                // Calculate weighted average price_per_unit and cost in original currency
                if (isset($buy_trades[$lot->buy_trade_id])) {
                    ...
                    // cost_original = cost_eur / fx_rate_to_eur (if currency is not EUR)
                    if ($buy_trade->trade_currency !== 'EUR') {
                        $fx_rate_to_eur = $buy_trade->fx_rate_to_eur;
                        if ($this->compare_decimals($fx_rate_to_eur, '0') > 0) {
                            $lot_cost_original = $this->divide_decimals($lot_remaining_cost, $fx_rate_to_eur);
                            $total_cost_basis_original = $this->add_decimals($total_cost_basis_original, $lot_cost_original);
                        } else {
                            // If fx_rate is 0, keep EUR value
                            $total_cost_basis_original = $this->add_decimals($total_cost_basis_original, $lot_remaining_cost);
                        }
                    } else {
                        // If EUR, cost in original currency equals EUR cost
                        $total_cost_basis_original = $this->add_decimals($total_cost_basis_original, $lot_remaining_cost);
                    }
                }
            }
            
            // If currency is EUR, ensure cost_original matches cost_eur
            if ($cost_currency === 'EUR') {
                $total_cost_basis_original = $total_cost_basis_eur;
            }
            ...
            // Get price values
            $close_price = $latest_price !== null ? $latest_price->close_price : '0';
            $previous_close_price = $previous_price !== null ? $previous_price->close_price : '0';
            $price_date = $latest_price !== null ? $latest_price->price_date : null;

            // Calculate price change
            $change = $this->subtract_decimals($close_price, $previous_close_price);
            $change_percent = '0.00';
            if ($previous_price !== null && $this->compare_decimals($previous_close_price, '0') > 0) {
                $change_percent = $this->multiply_decimals(
                    $this->divide_decimals($change, $previous_close_price),
                    '100'
                );
            ...
            $value_eur = $this->multiply_decimals($shares, $close_price);
            $value_eur = $this->round_decimal($value_eur, 2);
            ...
            if ($this->compare_decimals($shares, '0') > 0) {
                ...
                $avg_cost = $this->round_decimal($avg_cost, 8);
            }
            
            // Round cost in original currency
            $total_cost_basis_original = $this->round_decimal($total_cost_basis_original, 2);

            // Calculate today's gain
            $todays_gain_eur = $this->multiply_decimals($change, $shares);
            $todays_gain_eur = $this->round_decimal($todays_gain_eur, 2);

            $todays_gain_percent = '0.00';
            if ($this->compare_decimals($total_cost_basis_eur, '0') > 0) {
                $todays_gain_percent = $this->multiply_decimals(
                    $this->divide_decimals($todays_gain_eur, $total_cost_basis_eur),
                    '100'
                );
            ...
            $total_change_eur = $this->subtract_decimals($value_eur, $total_cost_basis_eur);
            $total_change_percent = '0.00';
            if ($this->compare_decimals($total_cost_basis_eur, '0') > 0) {
                $total_change_percent = $this->multiply_decimals(
                    $this->divide_decimals($total_change_eur, $total_cost_basis_eur),
                    '100'
                );
                $total_change_percent = $this->round_decimal($total_change_percent, 2);
            }

            // Calculate sell 100% tax (Slovenia)
            $sell_date = $price_date ?? date('Y-m-d');
            $sell_100_tax_eur = $this->compute_sell_100_tax_si($lots, $close_price, $sell_date);
            ...
            $holdings[] = [
                ...
                'lots' => $lots,
                'shares' => $shares,
                'cost_basis_eur' => $this->round_decimal($total_cost_basis_eur, 2),
                'cost_basis_original' => $total_cost_basis_original,
                'cost_currency' => $cost_currency,
                'close_price' => $close_price,
                'previous_close_price' => $previous_close_price,
                'change' => $this->round_decimal($change, 2),
                'change_percent' => $change_percent,
                'avg_cost' => $avg_cost,
                'value_eur' => $value_eur,
                'todays_gain_eur' => $todays_gain_eur,
                'todays_gain_percent' => $todays_gain_percent,
                'total_change_eur' => $this->round_decimal($total_change_eur, 2),
        ...
        foreach ($holdings as $holding) {
            $total_portfolio_value = $this->add_decimals($total_portfolio_value, $holding['value_eur']);
        }
        $total_portfolio_value = $this->round_decimal($total_portfolio_value, 2);

        // Add weight to each holding
        foreach ($holdings as &$holding) {
            $weight = '0.00';
            if ($this->compare_decimals($total_portfolio_value, '0') > 0) {
                $weight = $this->multiply_decimals(
                    $this->divide_decimals($holding['value_eur'], $total_portfolio_value),
                    '100'
                );
    ...
    public function compute_sell_100_tax_si(array $lots, string $sell_price, string $sell_date): string
    {
        ...
        // Total proceeds = total_shares * sell_price
        ...
        foreach ($lots as $lot) {
            ...
            // Cost basis for this lot (remaining portion)
